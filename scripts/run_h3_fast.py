"""EXP-20260902-002 (H3) on the ADR-0003 fast path — 96 frozen configs.

Usage:
    python scripts/run_h3_fast.py --period insample [--symbols ...]
    python scripts/run_h3_fast.py --period oos     # ONLY on explicit user go
...
from quantlab.research.fast.h3fast import (
    TF_1D,
    TF_4H,
    H3Config,
    pool_init_h3,
...
EXP_DIR = (
    Path(__file__).resolve().parent.parent / "experiments" / "EXP-20260902-002-h3-noise-sized-sweep"
)
TIMEFRAMES = [Timeframe.M15, Timeframe.H4, Timeframe.D1]  # MultiRow index order
CONTEXT_LABEL = {TF_4H: "4h_n5", TF_1D: "1d_n3"}
# frozen neighborhoods (experiment.json, 2026-09-02): 96 configurations
CONTEXTS = [TF_4H, TF_1D]
N_15M = [2, 3]
DETECTORS: list[tuple[int, float]] = [(0, 2.0), (1, 2.0)]  # (is_atr, mult) — frozen at mult 2
BUFFERS = [0.0, 0.1]
K_STOPS = [3.0, 4.0]
...
def h3_row(config: H3Config, m: FastMetrics, context: FastContext, days: int) -> dict[str, object]:
...
def run_symbol(url: str, symbol: str, period: str, workers: int) -> tuple[list[dict], float, float]:
    ...
    with ProcessPoolExecutor(
        max_workers=workers, initializer=pool_init_h3, initargs=(rows,)
    ) as pool:
        for shard_result in pool.map(run_shard_h3_pooled, shards):
            for index, metrics, context, _log in shard_result:
                results[index] = (metrics, context)
    compute_s = time.monotonic() - t1
    out = [h3_row(configs[i], *results[i], days) for i in range(len(configs))]
...
def main(argv: list[str]) -> int:
    ...
    print(f"== EXP-20260902-002 {args.period} (fast, {args.workers} workers, 96 configs) ==")
